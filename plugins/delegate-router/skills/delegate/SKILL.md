...
allowed-tools: Read, Grep, Glob, Agent, Skill, Bash(delegate-route:*), Bash(delegate-health:*), Bash(delegate-cursor:*), Bash(delegate-jobs:*), Bash(delegate-usage:*), mcp__delegate_control__delegate_start, mcp__delegate_control__delegate_inspect, mcp__delegate_control__delegate_list, mcp__delegate_control__delegate_stats, mcp__delegate_control__delegate_events, mcp__delegate_control__delegate_transcript, mcp__delegate_control__delegate_diff, mcp__delegate_control__delegate_files, mcp__delegate_control__delegate_steer, mcp__delegate_control__delegate_cancel, mcp__delegate_control__delegate_release, mcp__delegate_control__delegate_resume, mcp__delegate_control__delegate_review_round, mcp__delegate_control__delegate_usage, mcp__delegate_codex__codex, mcp__delegate_codex__codex-reply
---
...
1. Parse explicit provider, model, mode, scope, effort, approval, network, background, timeout, idempotency, retry, verification, and output-token budget controls. Explicit user choices win unless unavailable or unsafe.
...
- `delegate_list` to rediscover jobs when an ID is no longer in context (after compaction or in a new session). It reconciles orphans and returns compact summaries, newest first; filter by `groupId` or request a group summary for fan-out completion.
- `delegate_inspect` for lifecycle, capabilities, continuation IDs, `resumable`, `driftReport`, any failure `checkpoint`, the final `result`, verification verdict, and current revision. `resultText` is the provider-independent text of the final message; use `resultWindow` (`find`, `offset`, `maxChars`) or `delegate-jobs result` window flags for long output. A read-mode job flagged `resultSuspect: 'short-final-message'` likely narrated instead of delivering — resume it with "paste the full findings now". `delegate_list` rows carry `session` and `rootJobId` so resume chains group. Cursor plan-mode output arrives as ACP plan updates rather than message chunks, so read the plan from `result.plan` (also mirrored as `plan.updated` transcript events); `result.text` holds only the conversational messages. A running job whose worker process died is reconciled to `failed` with an `ORPHANED` error event.
...
For long jobs, do not chain polling turns: run `delegate-jobs wait <job-id>` with the Bash tool's `run_in_background: true` and continue other work — the harness notifies on exit (0 completed, 3 failed, 4 cancelled, 5 no observed changes, 6 verification failed, 75 wait-timeout). Bound the whole job with `timeoutSeconds` on `delegate_start` (60–86400; default 3600, deliberately long because LLM jobs legitimately run long). `maxOutputTokens` stops through the provider-aware cancel path and fails with `stoppedReason: "budget"` plus `BUDGET_EXCEEDED`, preserving partial state and the continuation ID.
...
Failure and cancellation records include a `checkpoint` with the reason, continuation ID, last diff event, and one-sentence resume guidance. Prefer its `resumeHint` over guessing. When an owned write must be rolled back, preview `delegate-jobs revert <id> --dry-run`, then run it without `--dry-run`; it restores only files still matching the job's recorded final hashes and refuses pre-existing overlap or later edits for manual resolution.

For a terminal fix/review round, use `delegate_review_round` (or `delegate-jobs review-round`) with the new findings. It refuses non-resumable parents and automatically anchors the continuation with the recorded diff stat, changed files, and scope. If stats predate the audit log, run `delegate-jobs audit backfill` once; it is idempotent.
