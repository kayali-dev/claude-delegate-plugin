...
At startup the TUI briefly measures a bounded set of width-sensitive graphemes with terminal cursor-position reports. Successful measurements are cached in private `<state-dir>/tui-prefs.json` entries keyed by `TERM`, terminal program/version, and whether tmux is interposed. `DELEGATE_TUI_WIDTH_PROBE=off` skips the measurement; `verbose` records its source, measured table, and elapsed time in the `M` status-history overlay. A terminal that does not answer silently uses the conservative static table after a sub-50 ms normal probe budget (with a hard ceiling below 100 ms).
...
| --- | --- |
| Codex app-server | Thinking, streaming, command/MCP/file tools, approval, required input, broker phase, quiet, stalled |
| Cursor ACP | Thinking, streaming, titled/location-bearing tools, approval, required input, broker phase, quiet, stalled |
| Cursor headless | Streaming, generic working with last-output age, quiet, stalled, terminal status |

Headless detail headers explicitly say `headless transport: reduced visibility`. They never infer thinking, tool, or approval state from signals that the transport cannot produce.
...
| `d` | From a Transcript tool with paths, open Diff filtered to those paths; in Launcher, build the dry run |
...
- Cursor headless has deliberately reduced live-activity visibility: partial output can show streaming/working age, but the transport cannot expose reasoning, structured tools, or approval waits.
